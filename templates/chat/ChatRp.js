...
import {getPartenaireById} from '../services/Services';
import ChatRpItem from './ChatRpItem';

class ChatRp extends Component {
	constructor(props) {
	  super(props);
	...
	componentDidMount(){
        //console.warn(this.state.ID_USER_2)
        getPartenaireById(this.state.ID_USER_2).then((data)=>{
            //console.warn(data)
            this.setState({
	...
	_renderItem(data){
		...
		return(
				<ChatRpItem conversations = {params} />
			);
	}
...
export default connect(mapStateToProps, { sendPartenaireMessage, sendMessageSyndic,  fetchConversationsPartenaire, fetchMessages})(ChatRp); 

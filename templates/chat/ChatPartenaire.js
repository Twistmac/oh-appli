...
import { sendPartenaireMessage, fetchConversationsPartenaire, fetchMessages, sendMessageSyndic } from '../actions';
import ChatPartenaireItem from './ChatPartenaireItem';
import firebase from '../../database/firebase';
import ChatItem from './ChatItem';
import {getResidentById} from '../services/Services';

class ChatPartenaire extends Component {
	constructor(props) {
	  super(props);
      ...
      //var detailResident = this.props.navigation.state.params.RESIDENT_DETAIL
      var InfoAnnonces = {
        id:dataAnnonces.id,
	...
	componentDidMount(){
        //console.warn(global.detailPartenaire[0].type)
        getResidentById(this.state.ID_USER_2).then((data)=>{
            //console.warn(data)
            this.setState({
                detailResident: data[0],
                ID_USER_2: data[0].id,
                USER_NAME_2: data[0].username
            })
          })
	...
	send(){
       ...
       const chats = {
            chatId: idChat,
            userName_1:global.detailPartenaire[0].username,
            userName_2:this.state.USER_NAME_2,
            senderId:user1,
            receivedId:user2,
            idSyndic:global.detailPartenaire[0].residence_id,
       ...
       const conversations = {
        ...
        this.props.sendPartenaireMessage(chats, conversations, idChat)
	...
	_renderItem(data){
        ...
        const params = {
            ...
            user2: global.detailPartenaire[0].id_partenaire
        }
        
		return(
				<ChatPartenaireItem conversations = {params} />
			);
	}
	...
	render() {
        ...
        {(global.detailPartenaire[0].type === 'p')?
  				<FlatList
            ref='list'
    				inverted
...
export default connect(mapStateToProps, { sendPartenaireMessage, sendMessageSyndic,  fetchConversationsPartenaire, fetchMessages})(ChatPartenaire); 
